let data = [
  {
    id: 1+0,
    key: "TheWindow",
    logo: "static/img/itemlogo_1.svg",
    title: "窗",
    sponsor: null,
    description: "以日常的觀察出發，將白天置於草地上的一扇窗，作為視野的出口，它透映出背後景象的真實身影，而當時間來到夜晚，燈光被點亮，窗即失去了穿透的特性，逐漸將夜晚的城市光點匯集成一扇光面，遮蔽住背後看不透的空無。",
    place: "北門大草坪",
    data: "清大學院招生與學習數據",
    data_description: "",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-Window.jpg",
    author: "三吉体飯糰",
    author_description: "三吉体飯團，由黃偉、羅佳柔、鄭羽芝三人組成，仍在廣泛學習中的跨領域創作新團體，從事作品乃在各式媒材中尋找出不一樣的觀看方式。",
    data_content: `<h2>01.〈窗〉 三吉体飯糰</h2>
<p><span style="color: #fff;">地點：北門大草坪</span><br>
<span style="color: #fff;">數據：清華招生與學習數據<br>
<span style="color: #fff;">清華學生學習表現數據分析平台是國內大學做的最完整之一，我們把教務處招生策略中心所提供的招生與學習數據，透過〈窗〉做整點的數據雕塑。我們好奇著清華在「學生人數比率、註冊率」、「多元學生組成狀況」、「男女 PK」、「社團活躍度」…等情形如何？〈窗〉將以「間格亮光」來作答回覆給我們。</span><br>
</span></p>
<p>清華「校本部8個學院」對應「窗格位置」如下：</p>
<table style="height: 177px; width: 191px;" border="1">
<tbody>
<tr>
<td style="width: 82.7833px;"><span>理學院</span></td>
<td style="width: 80.2167px;"><span>工學院</span></td>
</tr>
<tr>
<td style="width: 82.7833px;"><span>電機資訊學院</span></td>
<td style="width: 80.2167px;"><span>人文社會學院</span></td>
</tr>
<tr>
<td style="width: 82.7833px;"><span>原子科學院</span></td>
<td style="width: 80.2167px;"><span>科技管理學院</span></td>
</tr>
<tr>
<td style="width: 82.7833px;"><span>清華學院</span></td>
<td style="width: 80.2167px;"><span>生命科學院</span></td>
</tr>
</tbody>
</table>
<p><span>互動情境：<br>
</span>〈窗〉整點光亮模式（晚上 6:00~6:05, 6:30~6:35, 7:00~7:05, 7:30~7:35 ….&nbsp; 11:00~11:05）。<br>
資料，由清華大學教務處招生策略中心提供。</p>


<p><h4>問題 ②：106學年度清華大學學士班報到率高的人氣學院？</h4><br>
(註冊率高於校註冊率的學院)</p>
<p><!--------------------------------------------------------></p>
<table style="height: 177px; width: 195.15px;" border="1">
<tbody>
<tr>
  <td style="width: 85px; color: black;" bgcolor="yellow" ><span>理學院</span></td>
  <td style="width: 82.15px; color: black;" bgcolor="yellow" ><span>工學院</span></td>
</tr>
<tr>
  <td style="width: 85px; color: black;" bgcolor="yellow" ><span>電機資訊學院</span></td>
  <td style="width: 82.15px; color: black;" bgcolor="yellow" ><span>人文社會學院</span></td>
</tr>
<tr>
  <td style="width: 85px; color: black;" bgcolor="yellow" ><span>原子科學院</span></td>
  <td style="width: 82.15px; color: black;" bgcolor="yellow" ><span>科技管理學院</span></td>
</tr>
<tr>
  <td style="width: 85px;"><span>清華學院</span></td>
  <td style="width: 82.15px;"><span>生命科學院</span></td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
<table style="width: 546px;" border="1">
<tbody>
<tr>
<td style="width: 52.35px; text-align: center;"><span style="font-size: 12px;">&nbsp;</span></td>
<td style="width: 37.65px; text-align: center;"><span style="font-size: 12px;">全校</span></td>
<td style="width: 48px; text-align: center;"><span style="color: #fff; font-size: 12px;">理</span></td>
<td style="width: 45px; text-align: center;"><span style="color: #fff; font-size: 12px;">工</span></td>
<td style="width: 47px; text-align: center;"><span style="color: #fff; font-size: 12px;">電資</span></td>
<td style="width: 44px; text-align: center;"><span style="color: #fff; font-size: 12px;">人社</span></td>
<td style="width: 42px; text-align: center;"><span style="color: #fff; font-size: 12px;">原科</span></td>
<td style="width: 44px; text-align: center;"><span style="color: #fff; font-size: 12px;">科管</span></td>
<td style="width: 48px; text-align: center;"><span style="font-size: 12px;">清華</span></td>
<td style="width: 48px; text-align: center;"><span style="font-size: 12px;">生科</span></td>
</tr>
<tr>
<td style="width: 52.35px; text-align: center;"><span style="font-size: 12px;">註冊率</span></td>
<td style="width: 37.65px; text-align: center;"><span style="font-size: 12px;">95.76%</span></td>
<td style="width: 48px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">98.82%</span></td>
<td style="width: 45px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">97.12%</span></td>
<td style="width: 47px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">96.46%</span></td>
<td style="width: 44px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">96.37%</span></td>
<td style="width: 42px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">97.65%</span></td>
<td style="width: 44px; text-align: center;"><span style="color: #ff0000; font-size: 12px;">96.52%</span></td>
<td style="width: 48px; text-align: center;"><span style="font-size: 12px;">93.88%</span></td>
<td style="width: 48px; text-align: center;"><span style="font-size: 12px;">89.22%</span></td>
</tr>
</tbody>
</table>
<p>資料來源：統計處大專校院系所特色及新生註冊率查詢網站<a href="https://stats.moe.gov.tw/enrollweb/Default.aspx">https://stats.moe.gov.tw/enrollweb/Default.aspx</a></p>
<p>&nbsp;</p>
`
  }, {
    id: 1+1,
    key: "Needlework",
    logo: "static/img/itemlogo_2.svg",
    title: "針線作品",
    sponsor: null,
    description: "運用了許多現代與當代藝術的造型元素，兼具實用性與藝術性的針線作品，創作過程必須恪守手工針織的工藝技法，包括各種針法、疏縫、壓線、滾邊以及幾何符號的安排。每一個在作品上的抽象幾何符號，都蘊含了一段創作者的生命歷程與故事，透過構圖與色彩的鋪陳，呈現作品獨特的個人風格，也建構一個理性與感性並存，反應創作者自身生命狀態的詩意象限。",
    place: "旺宏圖書館",
    data: "清華園圖像數據",
    data_description: "清華創校61年，有著輝煌的歷史與記憶。我們把記藝數字植入光中，用 Bar-code方式呈現，猶如燈光密碼，植光計畫用來裝飾校園，也喚起清華人對校園歷史的記憶。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/04/THE2018-sew2.jpg",
    author: "侯王淑昭",
    author_description: "侯王淑昭，1978年創辦「春之藝廊」，影響台灣當代藝術發展深遠；1999年創辦「財團法人春之文化基金會」，同年創立「橘園國際藝術策展公司」策劃規劃國內公共藝術案；2012年創設「財團法人東和鋼鐵文化基金會」，邀請國內外藝術家進駐廠區創作，締造藝術與工業的全新連結。侯王女士的藝術實踐顯露出她對藝術的熱情與堅持，同時亦積極關注台灣現當代藝術發展，進而擔任當代藝術基金會董事長，積極活動於國際性組織如亞洲文化協會和國家文化藝術基金會等，鼓勵藝術家、策展人參與國際交流；於2015年獲教育部頒贈「社教貢獻獎」。",
    data_content: `<p><span class='orange-sub-title'><span class='orange-sub-title'>互動情境：</span><br>
<span class='orange-sub-title'>① 觀者立於作品前觀賞作品中的25個圖案，想像圖案與清華校園的關係。</span><br>
<span class='orange-sub-title'>② 並從我們提供的APP輸入你的想像。</span><br>
</span></p>
<p
註：請從APP，寫上你對作品圖案與清華的聯想。</p>`,
    appRoute: "/app/hohoho"
  }, {
    id: 1+2,
    key: "PlantingLight",
    logo: "static/img/itemlogo_3.svg",
    title: "植光計畫",
    sponsor: "盧崑瑞 思銳科技董事長 展出贊助",
    description: "在一棵大樹上架起象徵無限循環的莫比烏斯圓環，串接正反面的二維曲度，在衍序漸變的和諧光環下，開啟科技的光與植物體依附共生，開啟自然生命體解構出一席之地的討論。",
    place: "人事室馬路對面樹叢",
    data: "清華創校重要數字記憶",
    data_description: "清華創校61年，有著輝煌的歷史與記憶。我們把記藝數字植入光中，用 Bar-code方式呈現，猶如燈光密碼，植光計畫用來裝飾校園，也喚起清華人對校園歷史的記憶。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-PlantLight.jpg",
    author: "蔡奇宏",
    author_description: "蔡奇宏，人生中的轉捩點受到許多師長啟蒙，從資訊背景跨入新媒體藝術領域。主要創作乃將類比與數位以多種方式進行混合，並以電子控制、互動感測、聲音裝置與聲音表演等形式來探索不同媒材之間與情感的連結。近期創作從跨領域出發，進入新媒體劇場，著力於電子、機械、動力控制與燈光媒合。曾獲第八屆李國鼎科技藝術獎互動藝術組首獎。",
    data_content: `<p><span class='orange-sub-title'>地點：人事室馬路對面樹叢</span><br>
<span class='orange-sub-title'>數據：清華創校重要數字（記憶）</span><br>
清華創校61年，有著輝煌的歷史與記憶。我們把記藝數字植入光中，用 Bar-code方式呈現，猶如燈光密碼，植光計畫用來裝飾校園，也喚起清華人對校園歷史的記憶。</p>
<p><span class='orange-sub-title'>互動情境：</span>（整點模式）<br>
① 光棒呈現Bar-Code條碼狀，表示整點數據。<br>
② 下面範例，1代表光棒「亮」，0代表光棒「暗」，閃代表光棒「閃爍」。<br>
③ 數字數據模式有兩種：年代(11)與日期(10)，兩個閃爍光棒代表數字數據的起，一個閃爍光棒代表數字數據的迄。<br>
&nbsp;範例：<br>
<img class="alignnone wp-image-1921" src="http://techart.nthu.edu.tw/wp-content/uploads/2018/04/03_Light-barcode.jpg" alt="" width="412" height="192" srcset="http://techart.nthu.edu.tw/wp-content/uploads/2018/04/03_Light-barcode.jpg 649w, http://techart.nthu.edu.tw/wp-content/uploads/2018/04/03_Light-barcode-465x216.jpg 465w" sizes="(max-width: 412px) 100vw, 412px"><br>
註：整點模式（晚上 6:00~6:05, 6:30~6:35, 7:00~7:05, 7:30~7:35 ….&nbsp; 11:00~11:05）</p>`
  }, {
    id: 1+3,
    key: "PrismaticColor",
    logo: "static/img/itemlogo_4.svg",
    title: "光譜原色",
    sponsor: "蔡能賢 台灣機體電路製造公司副總經理 展出贊助",
    description: "光譜原色時代，在多彩的日子裡，色彩仍反應著美好的年代。光象徵真理，而發散美麗光彩的稜鏡就像闡釋真理的心智。透過幾何稜鏡產生光譜的原色色散，提示著真理的清晰與曖昧難明，作品引用美國詩人瑪麗安娜‧摩爾的詩歌，闡述單純的美好，今日在紛雜的普羅萬世裡迷茫，然而真理本身依然如昔。幾何形體的稜鏡透過燈光的照射閃爍著如霓虹的光芒，照映在一日多變的陽光下，色彩如四季變遷。",
    place: "成功湖上",
    data: "參與者點燈互動行為數據",
    data_description: "民眾於湖畔欣賞光譜原色，可透過手機頁面來對五個三角立錐做光譜點燈，而對湖（手機）唱歌時，三角立錐會閃爍不停。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-LightSpectrum.jpg",
    author: "蔡宜婷",
    author_description: "蔡宜婷，從雕塑出發結合新媒體應用創作，創作多元，嘗試以雕塑物件以及機械動力裝置提煉一種生命運動中的恆定狀態，透過冷調與理性探討人類無謂勞動與異化的過程，進而以時間空間為基底發展延伸。近期的光藝術創作則探索環境與人的歸依、光的色析與回歸，在幾何構成中精準玩轉光影色彩。曾獲2015年國立臺北藝術大學新媒體藝術創作卓越獎、2016年桃源創作獎首獎，並受歐洲藝術學院聯盟邀請參與荷蘭阿姆斯特丹NEU NOW 2016藝術節。",
    data_content: `
      <p><span class='orange-sub-title'>互動情境：</span><br>
①&nbsp; 剛開始時，五個湖上的三角立錐微亮狀態。<br>
② 參與者從手機APP，可點選三角立錐做點燈互動，然後按 Go「點燈」。<br>
③ 所有互動停止10秒後，五個立錐全部又回覆到微亮的初始狀態。<br>
    `,
    appRoute: "/app/prismatic"
  }, {
    id: 1+4,
    key: "Mushrooms",
    logo: "static/img/itemlogo_5.svg",
    title: "蘑菇總動員",
    sponsor: "蔡能賢 台灣機體電路製造公司副總經理 展出贊助",
    description: "這曾是台灣第一個以無線感測網路技術應用於人文生活休憩空間的互動數位公共藝術，改製實現於清華校園中。音樂模式，參與者在蘑菇樹道行走用其「行走行為」為樹道作曲，行人猶如「生活創意指揮家」用行走來創意指揮著音樂樹道，讓校園生活中充滿「環境感知互動美學」。讓大家於校園生活中享受數位創意賞析，感受高科技親民的一面。此作品使清華校園生活充滿環境感知互動美學。",
    place: "成功湖畔樹叢",
    data: "對清華印象的話語",
    data_description: "〈蘑菇總動員〉另一模式為語彙模式，參與者於作品提供的APP留下對清華的印象文字或聲音，爾後民眾穿梭於蘑菇樹道，可聽到這些語彙，聽聽大家在聊什麼、在關心什麼，此時樹道形成一個同步聲繪的數位公共藝術作品，它既是件藝術作品也是清華人的印象聲音。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-mushroom.jpg",
    author: "許素朱（FBI Lab)、劉士達、吳柏瑤",
    author_description: "<u>許素朱</u>， 1992年曾在台北藝術大學創立台灣最早跨領域的「科技藝術研究所」、2013年成立「台灣科技藝術學會」、1998年主持FBI Lab至今，從事科技藝術創作與研究26年，長年扮演台灣科技與藝術結合的橋樑。興趣，浪跡天涯；志向，當俠女。<br><u>劉士達</u>，曾創辦台灣Arduino.TW網站、擔任過2017台北世大運開幕表演道具技術總監，以及擔任2014、2015年工研院解密國家寶藏－奇想樂園互動科技體驗館之系統開發總監，技術跨域整合的實務經驗豐富，現為清華大學科技藝術研究中心組長與藝術學院學士班講師。<br>    <u>吳柏瑤</u>，大學就讀國立清華大學不分系美術組，後分流至資訊工程系。從小喜愛繪畫，熱衷創作。雖讀理工，仍不放棄對藝術的熱愛。近期成功進入清大跨院碩藝術與創新科技組，未來將朝科技藝術跨域創作與研究。",
    data_content: `
      <p><span class='orange-sub-title'>音樂蘑菇互動情境：</span><br>
① 不管哪一朵蘑菇，當第一位民眾靠近蘑菇時，該蘑菇會播放「波麗露」主旋律音樂。<br>
② 爾後民眾靠近其他蘑菇時，蘑菇會播放它所負責「波麗露」副旋律音樂。<br>
③ 所有人都離開時，蘑菇會自動停止音樂播放。</p>
<p><span class='orange-sub-title'>語彙蘑菇互動情境：</span><br>
① 民眾靠近任何一朵蘑菇時，都可聽到儲存在雲端大家對清華的印象的話語。<br>
② 民眾也可在我們提供的APP 錄下(最多 10秒）或寫下(最多 40字）對清華的印象。<br>
註：請填上你的學號與聯絡電話，作為未來抽獎時聯繫。</p>
    `,
    appRoute: "/app/mushroom"
  }, {
    id: 1+5,
    key: "GeometricInterspace",
    logo: "static/img/itemlogo_6.svg",
    title: "幾何空隙",
    sponsor: null,
    description: "以日常不可見的光線與電波訊號或空間縫隙作為出發點，作品探討如何在有機的環境中，展現那些支配著環境的訊號與粒子穿梭在場域中，利用自然中不存在的幾何與有機形體作為對照，透過完形理論，抓住人對於光線的剎那暫留，讓有機體與裝置之間產生交錯，藉此探討空間與縫隙之間的稍縱即逝。",
    place: "小吃部對面樹叢",
    data: "清華圖書館進館人數",
    data_description: "清華圖書館有著最雅致的閱讀空間、最豐富的館藏，是清華人最常駐足的公共空間。作品中的三個立體矩形分別代表工學院、理學院、人文社會學院每天學生進入圖書館的人數總和，立體矩形每一邊燈管亮代表20人進圖書館。藉此，我們探索著三個學院每天進入圖書館的人數。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-geometrix.jpg",
    author: "陳威諭",
    author_description: "陳威諭，作品涉及訊號、機械、人工智慧等，嘗試找尋科技內的細微現象，關注科技對人類行為的影響，以人機互動、演算法介入日常空間，探討一種資訊、虛構融入生活空間的未來想像。",
    data_content: `<p><span class='orange-sub-title'>互動情境：<span>（整點模式）</span></span><br>
① 立體矩形，Led以彩色發光，代表整點數據呈現。<br>
②每一邊燈管亮代表15人進圖書館，一個立體矩形全亮，代表當天約210人進圖書館。若三個立體矩形全亮，則代表當天約 630人進圖書館。<br><br>
註：整點模式（晚上 6:00~6:05, 6:30~6:35, 7:00~7:05, 7:30~7:35 ….&nbsp; 11:00~11:05）</p>
`
  }, {
    id: 1+6,
    key: "PressureEffects",
    logo: "static/img/itemlogo_7.svg",
    title: "力光見影",
    sponsor: null,
    description: "運用「壓力」的強弱與燈光裝置互動，將「力的傳達」同步轉化為顯現於地板之光影強弱的效果－參觀者可以身體之重量坐壓燈光裝置椅凳，當坐壓力量越大，裝置顯現的光度越強，投射地面光影也越大。作品以「光影」為介質，將無形的「壓力」轉化為有形的光影效果，壓力之下，光芒四射 – 力光見影。",
    place: "綜合二館穿堂",
    data: "參與〈力光見影〉愉悅指數",
    data_description: "參觀者參與本作品坐壓於光椅凳時，不但會影響地板光度與地面光影大小，同時也即時的影響〈英文8-2〉切換至娛悅模式，創造綜合大樓廣場的歡愉氣氛。大家多到校園走走，體驗校園的美與舒暢。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018_Strength.jpg",
    author: "林靖祐",
    author_description: "林靖祐，建築照明設計師，「沁弦‧築影」設計總監。旅美期間曾參與多項大型國際知名建案照明設計，作品觸角多元，2013年底返臺開業後至今已完成多項建築照明、室內、展場燈光與燈光裝置藝術等作品。對「光」具有專業的理性與感性的敏銳度，認為「光」不僅是功能性的照明，亦可是形塑意象與在地文化結合的藝術。",
    data_content: `<p><span class='orange-sub-title'>互動情境：</span><br>
①參觀者坐壓於光椅凳時，影響地板光度與地面光影大小。<br>
②參觀者坐壓於其中一把光椅凳時，也會影響對面〈英文8-2〉切換至愉悅模式，十個光柱會做隨機多彩發光秀。<br>`
  }, {
    id: 1+7,
    key: "English82",
    logo: "static/img/itemlogo_8.svg",
    title: "英文8-2",
    sponsor: "財團法人自強工業科學基金會 展出贊助",
    description: "光，在生活空間中可以是娛悅的、浪漫的、哲學的存在，但也可以是理性對話的存在。我們試圖在廣場空間製作一道可以演繹不同時空氛圍的光柱雕塑，顛覆空間視覺，創造想像力穿梭；而在整點定時也詮釋著無所不在圍繞著我們身邊的數據，讓空間可沈思。",
    place: "兩綜合大樓中間廣場",
    data: "清華學習英文成績",
    data_description: "清華大學部學生需修滿8學分，亦可通過某些鑑定考試免2學分。我們希望藉由我們提供的「APP英文學習」，透過物聯網傳輸英文學習成績至綜合館廣場的代表各院的藝術燈，於整點時點亮英文成績。盼大家的努力，可減2學分。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-Eng8-2.jpg",
    author: "劉士達、許素朱、吳哲宇",
    author_description: "<u>劉士達</u>，畢業於臺北藝術大學科技藝術研究所，曾創辦台灣Arduino.TW網站，是台灣Arduino技術創意開發最具代表性人物之一，亦曾擔任過2017台北世大運開幕表演道具技術總監，以及擔任2014、2015年工研院解密國家寶藏－奇想樂園互動科技體驗館之系統開發總監，技術跨域整合的實務經驗豐富，現為清華大學科技藝術研究中心組長與藝術學院學士班講師。<br><u>吳哲宇</u>，交大電機系畢業，喜歡視覺設計與程式撰寫，是設計與網頁雙棲工程師。大二創立墨雨設計，致力結合動態精神於網頁中，以程式創作動畫與特效，亦嘗試開設線上課程做設計整合程式導向教學。近期轉往互動裝置藝術裝置領域探索，盼能找到更具跳戰的媒材與感動。",
    data_content: `<p><span class='orange-sub-title'>互動情境：<span>（整點模式）</span></span><br>
①整點英文數據模式：十根Led光柱代表不同學院，每天定點出現每個學院的英文學習累積成績。（本作品提供英文學習APP，如下。）</p>
<p>在非整點模式，還有三個互動模式：<br>
②抒情模式：我們用Led光條發光高低呈現，以光波形做光藝術表演。<br>
③藝術模式：我們用Led光的閃爍，以光節奏做光藝術表演。<br>
④愉悅模式：只要〈力光見影〉有人參與坐壓愉悅舒緩，便可讓〈英文8-2〉切換至愉悅模式，此時Led光柱會做隨機多彩顏色發光。<br>
註：整點模式（晚上 6:00~6:05, 6:30~6:35, 7:00~7:05, 7:30~7:35 ….&nbsp; 11:00~11:05）</p>`
  }, {
    id: 1+8,
    key: "QuasiLand",
    logo: "static/img/itemlogo_9.svg",
    title: "草原座標",
    sponsor: null,
    description: "本作品利用日常用以度量空間的水平鐳射光置入草原中，並以其照射難以被度量的景物中，透過橫向的光線切割在垂直生長的草原，構成「座標」，讓空間重新被配置，鐳射光打在草產生光的蠕蟲攀爬於草枝上，此時光線與被投影的草原產生一種互相依附關係，進而產生一種由清晰光線掃描產生的混亂視覺空間。",
    place: "南門紫荊葉公共藝術旁草原",
    data: "清華穿梭南門車流量",
    data_description: "機械結構每天循環的運作著，但隨著時間與作品前參與者以及路旁車輛行駛通過，這些介入改變了鐳射光的束量與強弱，讓光蠕蟲被激發，使眼前草原風景變得會因時而異，整個草域變成了由自然與人工所中介且共構出的體驗空間。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-Grass.jpg",
    author: "高德亮",
    author_description: "高德亮，作品常以光作為創作的主要材料，探討著科技發展產生的空間感知與觀看經驗的流變。在日趨同質化與複製化的生活環境中，試圖透過創作的介入與物件的借用，提出有別於日常的感知經驗，並透過「隨機」、「混亂」、「無序」等關鍵詞的思考作為突破日常生活經驗的主要手段。",
    data_content: `<p><span class='orange-sub-title'>互動情境：</span><br>
①清華南門車輛流量，流量越大啟動了鐳射光的束量。<br>
②〈風極體〉隨風搖曳與閃爍發光越大，影響鐳射光發射頻率，激發雷射光蠕蟲活力越大。<br>`
  }, {
    id: 1+9,
    key: "Windiode",
    logo: "static/img/itemlogo_10.svg",
    title: "風極體",
    sponsor: null,
    description: "作品在每一隻碳纖棒上都配置一個簡單平衡儀和一個LED，當風力影響碳纖棒傾斜時LED就會閃爍。五百隻複製碳纖棒，隨風產生一個風極體的擺盪。由於風力和碳纖棒的軔性構築了閃爍的時序，讓整個風極體隨機閃爍，從中可觀察到一種構築在隨機性上的韻律和節奏美。",
    place: "台積館門口廣場",
    data: "新竹風",
    data_description: "新竹風大遠近馳名，有風城之稱。風極體是件隨風搖盪光閃爍的公共藝術，同時也是件風力偵測器，透過Led燈變化反應不同的風力大小。風極體，隨風搖曳，點綴了南校區。",
    cover: "http://techart.nthu.edu.tw/wp-content/uploads/2018/03/THE2018-Wind-1.jpg",
    author: "施惟捷",
    author_description: "施惟捷，媒體藝術工作者、材料研究者。2011起開始專注在電子織品的材料上，其穿戴式作品陸續在各國際電子織品和創客平臺上受到關注和邀約。作品「雷射染／Laser Dye」利用雷射感光放大傳統攝影沖洗底片過程，將光的物理特性作更深部的解構，並運用在視覺藝術上和成衣印花的數位製造上。曾擔任臺灣電子織品與民族織品合作平臺－「部落對抗機器」活動的主辦人，探索新媒體如何在民族社群中做為社會工具之意義。",
    data_content: `<p><span class='orange-sub-title'>互動情境：</span><br>
①風靜，風極體直豎、Led燈不閃爍；風越大，風極體隨風搖曳、Led燈閃爍不停。<br>
②風極體，隨風搖曳、Led隨風閃爍，會影響〈草原座標〉雷射光蠕蟲的活動力。<br>`
  }
]

export default data